#!/usr/bin/env python3
"""Helpers for building a Kaggle-compatible custom metric artifact."""
...
def score_rows(solution_rows: Sequence[Dict[str, object]], submission_rows: Sequence[Dict[str, object]]) -> float:
    if len(solution_rows) != len(submission_rows):
        return 0.0
...
def write_metric_artifact(output_path: Path) -> Path:
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(build_metric_source(), encoding="utf-8")
    return output_path
